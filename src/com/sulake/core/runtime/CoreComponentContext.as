package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1507:String = "asset-libraries";
      
      private static const const_1513:String = "library";
      
      private static const const_1514:String = "service-libraries";
      
      private static const const_1512:String = "library";
      
      private static const const_1511:String = "component-libraries";
      
      private static const const_1506:String = "library";
      
      private static const const_1509:String = "error_data";
      
      private static const const_1510:String = "error_cat";
      
      private static const const_1508:String = "error_desc";
       
      
      private var var_385:LibraryLoaderQueue;
      
      private var var_811:IEventDispatcher;
      
      private var var_536:uint;
      
      private var var_384:Function;
      
      private var var_220:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1294:
               debug("Core; using simple frame update handler");
               this.var_384 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_1020:
               debug("Core; using complex frame update handler");
               this.var_384 = this.complexFrameUpdateHandler;
               break;
            case Core.const_919:
               debug("Core; using profiler frame update handler");
               this.var_384 = this.profilerFrameUpdateHandler;
               this.var_220 = new Profiler(this);
               attachComponent(this.var_220,[new IIDProfiler()]);
               var_431.addChild(new ProfilerViewer(this.var_220));
               break;
            case Core.const_493:
               debug("Core; using experimental frame update handler");
               this.var_384 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_384 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_536 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1509,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1510,String(param3));
         ErrorReportStorage.setParameter(const_1508,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_811 = loadingEventDelegate;
         if(this.var_385 == null)
         {
            this.var_385 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1507)[0];
            if(node != null)
            {
               list = node.child(const_1513);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_231,e);
         }
         try
         {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_231,e);
         }
         try
         {
            node = config.child(const_1511)[0];
            if(node != null)
            {
               list = node.child(const_1506);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_231,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1339);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_811)
         {
            this.var_811.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_811)
         {
            ...
            this.var_811.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_811)
                  {
                     this.var_811.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_536 - this.var_385.length,this.var_536,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_220 ? 0 : this.var_1515 & 0;
         if(_loc3_ == Core.const_493)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_384(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_649[priority] < priority)
               {
                  ++this.var_649[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_220.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_220)
            {
               detachComponent(this.var_220);
               this.var_220.dispose();
               this.var_220 = null;
            }
            switch(this.var_1515 & 0)
            {
               case Core.const_1294:
                  this.var_384 = this.simpleFrameUpdateHandler;
                  break;
