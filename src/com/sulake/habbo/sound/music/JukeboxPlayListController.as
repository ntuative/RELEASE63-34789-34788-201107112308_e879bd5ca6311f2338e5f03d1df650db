package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_844 = [];
         this.var_844.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_844.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_844.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         return HabboMusicPrioritiesEnum.const_124;
      }
      ...
      {
         this.var_263.stop(this.priority);
         this.var_498 = -1;
         this.var_1938 = -1;
         this.var_734 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_734 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_263.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_124,_loc3_.syncCount / 1000,0,1,1);
            this.var_498 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_263.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1938 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_102,HabboMusicPrioritiesEnum.const_124,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_263.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               ...
               if(this.var_843.indexOf(_loc5_) < 0)
               {
                  ...
                  this.var_263.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_547.push(_loc7_);
            _loc4_++;
         }
      ...
      private function onJukeboxPlayListFullMessage(param1:IMessageEvent) : void
      {
         ...
         this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_165));
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_547[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_263.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_547[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
