package com.sulake.habbo.toolbar
{
   ...
   public class ExtensionView implements IExtensionView
   {
      ...
      private var var_139:IItemListWindow;
      ...
      public function ExtensionView(param1:IHabboWindowManager, param2:IAssetLibrary, param3:HabboToolbar)
      {
         ...
         if(_loc4_)
         {
            this.var_139 = param1.buildFromXML(_loc4_.content as XML,1) as IItemListWindow;
         }
         var _loc5_:IWindowContainer = this.var_139.desktop;
         if(this.var_139)
         {
            this.var_139.x = _loc5_.width - this.var_139.width - const_1146;
            this.var_139.y = const_1146;
            this.var_139.visible = true;
         }
         else
         {
            Core.error("Unable to initialize Toolbar Extension view window from xml asset",false,Core.const_1362);
         }
         this._items = new Map();
      }
      ...
      public function get screenHeight() : uint
      {
         ...
         return this.var_139.height + this.var_139.y;
      }
      ...
      public function attachExtension(param1:String, param2:IWindow, param3:int = -1) : void
      {
         ...
         if(this.var_139)
         {
            if(param3 == -1)
            {
               this.var_139.addListItem(param2);
            }
            else
            {
               this.var_139.addListItemAt(param2,param3);
            }
            this.var_139.invalidate();
         }
      ...
      public function detachExtension(param1:String) : void
      {
         ...
         if(this.var_139)
         {
            this.var_139.removeListItem(_loc2_);
         }
      ...
      private function onResizeTimer(param1:TimerEvent) : void
      {
         if(this._toolbar)
         {
            this._toolbar.events.dispatchEvent(new ExtensionViewEvent(ExtensionViewEvent.const_1375));
         }
      }
   }
}
