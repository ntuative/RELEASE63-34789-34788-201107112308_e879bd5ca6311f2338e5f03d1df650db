package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_381:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_381 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this._figure && this.var_1490)
         {
            _loc5_ = this.var_26.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_109,this.var_991,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_41,parseInt(FigureData.const_1013));
         ...
         if(this.var_381)
         {
            if(this.var_381.bitmap)
            {
               this.var_381.bitmap.dispose();
            }
            this.var_381.bitmap = new BitmapData(this.var_381.width,this.var_381.height,true,0);
            _loc7_ = (this.var_381.width - _loc1_.width) / 2;
            _loc8_ = (this.var_381.height - _loc1_.height) / 2;
            this.var_381.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_991 = this.var_26.gender;
               this.var_26.handler.saveWardrobeOutfit(this.var_1833,this);
               this.updateView();
               break;
            case "get_button":
               if(this._figure)
               {
                  this.var_26.loadAvatarInEditor(this._figure,this.var_991,this.var_26.clubMemberLevel);
               }
         }
      }
