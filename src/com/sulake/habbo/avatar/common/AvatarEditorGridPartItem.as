package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_910:IFigurePartSet;
      ...
      private const const_2369:int = 13421772;
      
      private const const_2368:int = 16777215;
      
      private const const_393:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2145 = Math.max(this.var_2145,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_754 != null && !this.var_2144)
            {
               _loc3_ = this.var_754;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_910)
            {
               switch(this.var_910.clubLevel)
               {
                  case HabboClubLevelEnum.const_52:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_980;
                     break;
                  case HabboClubLevelEnum.const_42:
                     _loc2_.style = HabboIconType.const_1005;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1517.color = this.const_2369;
         }
         else
         {
            this.var_1517.color = this.const_2368;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_393[_loc6_] + "_" + FigureData.const_153;
               _loc4_ = this.var_497.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_393.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_393[_loc6_] + "_" + FigureData.const_153;
                  _loc4_ = this.var_497.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_733 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2143)
               {
                  ...
                  this.var_2143 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_733.width,this.var_733.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_733.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_733.y;
               if(this.var_2144 && _loc2_.colorLayerIndex > 0)
               {
