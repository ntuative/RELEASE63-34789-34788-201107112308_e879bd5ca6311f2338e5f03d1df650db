package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private static var var_1207:int = 1;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         var _loc2_:int = this.var_155.indexOf(param1);
      ...
      public function closeConversation() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc2_ = this.var_155.indexOf(_loc1_);
            Util.remove(this.var_155,_loc1_);
            this._closedConversations.push(_loc1_);
            _loc1_.setSelected(false);
            while(_loc2_ >= 0)
            {
               if(this.var_155[_loc2_] != null)
               {
                  this.var_155[_loc2_].setSelected(true);
                  break;
               }
               _loc2_--;
            }
      ...
      {
         this.addConversation(var_1207++);
      }
      
      public function addConversation(param1:int) : Conversation
      {
         var _loc2_:Boolean = this._closedConversations.length == 0 && this.var_155.length == 0;
         var _loc3_:Conversation = this.addConversationInt(param1);
         if(_loc3_ == null)
         {
            return null;
         }
         if(this.var_155.length == 1)
         {
            this.var_155[0].setSelected(true);
         }
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_1019,0,this.var_441.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:int = this.var_155.length;
         ...
         if(_loc2_ != this.var_155.length || _loc4_ != _loc3_.newMessageArrived)
         {
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_155.length - this.var_441.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_803,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
