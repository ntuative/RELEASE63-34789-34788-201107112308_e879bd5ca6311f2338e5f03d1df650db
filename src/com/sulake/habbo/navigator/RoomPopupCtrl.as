package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_145:GuestRoomData;
      
      private var var_2812:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2812 = new RoomThumbnailRenderer(param1);
         this.var_1082 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_211 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_211.visible = true;
         Util.hideChildren(this.var_211);
         this.refreshRoomName(this.var_211,this.var_145);
         if(this.var_145.ownerName != "" && this.var_145.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_211,!!this.var_145.event ? this.getEventCtg(this.var_145.categoryId) : this.getRoomCtg(this.var_145.categoryId));
         this.refreshRoomDesc(this.var_211,this.var_145);
         this.refreshExtraCont();
         this.refreshInfo(this.var_211,"trading_allowed",this.var_145.allowTrading);
         this.refreshInfo(this.var_211,"doormode_doorbell",this.var_145.doorMode == 1);
         this.refreshInfo(this.var_211,"doormode_password",this.var_145.doorMode == 2);
         Util.moveChildrenToColumn(this.var_211,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_211.height = Util.getLowestPoint(this.var_211);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_211,this.var_145.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_211.findChildByName("roomowner"));
         if(this.var_145.ownerName != "" && this.var_145.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_211.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1082.refreshTags(_loc1_,this.var_145.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_145.score);
         if(this.var_145.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_145.eventCreationTime);
         }
         this.var_2812.refreshThumbnail(_loc1_,this.var_145.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_304,Tabs.const_439,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
